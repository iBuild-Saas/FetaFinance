-- =====================================================
-- FIX SALES INVOICE SCHEMA MISMATCH
-- =====================================================
-- This script fixes the mismatch between UI expectations and database schema

-- =====================================================
-- 1. CREATE SALES_INVOICE_LINE_ITEMS TABLE (UI expects this name)
-- =====================================================
...
CREATE TABLE IF NOT EXISTS sales_invoice_line_items (
    ...
    item_id UUID REFERENCES items(id), -- Link to items table for inventory
...
CREATE INDEX IF NOT EXISTS idx_sales_invoice_line_items_invoice_id ON sales_invoice_line_items(sales_invoice_id);
CREATE INDEX IF NOT EXISTS idx_sales_invoice_line_items_item_id ON sales_invoice_line_items(item_id);

-- =====================================================
-- 3. CREATE UPDATED_AT TRIGGER
-- =====================================================

CREATE OR REPLACE FUNCTION update_sales_invoice_line_items_updated_at()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at = NOW();
    RETURN NEW;
...
CREATE TRIGGER trigger_update_sales_invoice_line_items_updated_at
    BEFORE UPDATE ON sales_invoice_line_items
    FOR EACH ROW
    EXECUTE FUNCTION update_sales_invoice_line_items_updated_at();

-- =====================================================
-- 4. UPDATE SALES INVOICE AUTOMATION TO USE CORRECT TABLE
-- =====================================================

-- Update the sales stock movement function to use correct table name
...
BEGIN
    ...
    -- Process each line item in the sales invoice (FIXED TABLE NAME)
    FOR v_line_item IN 
        SELECT 
            item_id,
            quantity,
...
-- Update the sales journal entry function to use correct table name
...
BEGIN
    ...
    -- Calculate Cost of Goods Sold (FIXED TABLE NAME)
    FOR v_line_item IN 
        SELECT 
            item_id,
            quantity,
...
GRANT ALL ON sales_invoice_line_items TO authenticated;

-- =====================================================
-- 6. VERIFICATION
-- =====================================================

SELECT '=== SALES INVOICE SCHEMA FIXED ===' as status;
...
SELECT '=== READY TO TEST SALES INVOICES ===' as next_step;
