-- =====================================================
-- FIXED AUTO JOURNAL ENTRY SYSTEM USING COMPANY FIELDS
-- =====================================================
-- This script creates automatic journal entry generation for purchase and sales invoices
-- using the company table fields that are populated from account mappings
...
BEGIN
    ...
    -- Get accounts from company table fields (populated from account mappings)
    SELECT 
        COALESCE(default_inventory_account_id, default_expense_account_id),
    ...
    IF v_inventory_account_id IS NULL THEN
        RAISE EXCEPTION 'No default inventory or expense account configured for company %. Please configure accounts in Account Mappings and ensure they are synced to company table.', v_company_id;
    END IF;
    
    IF v_payable_account_id IS NULL THEN
        RAISE EXCEPTION 'No accounts payable account configured for company %. Please configure accounts in Account Mappings and ensure they are synced to company table.', v_company_id;
    END IF;
    ...
    RAISE NOTICE 'Created journal entry % for purchase invoice % using company default accounts', v_journal_number, NEW.invoice_number;
...
BEGIN
    ...
    IF v_revenue_account_id IS NULL THEN
        RAISE EXCEPTION 'No default sales revenue account configured for company %. Please configure accounts in Account Mappings and ensure they are synced to company table.', v_company_id;
    END IF;
    
    IF v_receivable_account_id IS NULL THEN
        RAISE EXCEPTION 'No default accounts receivable account configured for company %. Please configure accounts in Account Mappings and ensure they are synced to company table.', v_company_id;
    END IF;
    
    IF v_tax_amount > 0 AND v_sales_tax_payable_account_id IS NULL THEN
        RAISE EXCEPTION 'Sales tax exists but no sales tax payable account configured for company %. Please configure accounts in Account Mappings and ensure they are synced to company table.', v_company_id;
    END IF;
    ...
    RAISE NOTICE 'Created journal entry % for sales invoice % using company default accounts', v_journal_number, NEW.invoice_number;
...
SELECT '=== FIXED AUTO JOURNAL ENTRY SYSTEM CREATED ===' as status;
...
SELECT 'System uses company table fields that are populated from Account Mappings' as info2;
SELECT 'Make sure Account Mappings are configured and synced to company table' as info3;
