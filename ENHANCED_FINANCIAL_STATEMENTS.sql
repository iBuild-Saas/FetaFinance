-- Enhanced financial statements that fetch account balances for specific periods
-- Similar to general ledger and trial balance approach

DROP FUNCTION IF EXISTS get_income_statement(TEXT, DATE, DATE);
DROP FUNCTION IF EXISTS get_balance_sheet(TEXT, DATE);

-- Enhanced Income Statement - fetches revenue/expense account balances for period
...
BEGIN
    ...
    WITH account_balances AS (
        ...
        WHERE coa.account_type IN ('REVENUE', 'INCOME', 'EXPENSE', 'COST_OF_GOODS_SOLD', 'OTHER_INCOME', 'OTHER_EXPENSE')
...
-- Enhanced Balance Sheet - fetches asset/liability/equity balances as of date
...
SELECT 'Testing enhanced financial statement functions:' as test_message;

-- Test income statement with period balances
SELECT 'Income Statement (Period Balances):' as report_type;
SELECT * FROM get_income_statement('1754730703821', '2024-01-01', '2024-12-31') LIMIT 10;

-- Test balance sheet with cumulative balances
SELECT 'Balance Sheet (Cumulative Balances):' as report_type;
SELECT * FROM get_balance_sheet('1754730703821', '2024-12-31') LIMIT 10;

SELECT 'Enhanced financial statements created - similar to trial balance approach' as status;
