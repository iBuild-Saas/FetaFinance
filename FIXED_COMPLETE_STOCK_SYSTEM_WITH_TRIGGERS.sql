-- =====================================================
-- COMPLETE STOCK SYSTEM WITH AUTOMATIC TRIGGERS (FIXED)
-- =====================================================
...
-- View for current stock levels with item details (FIXED - removed is_active filter on items)
...
SELECT 
    ...
    sl.item_id,
    COALESCE(i.item_code, sl.item_id::text) as item_code,
...
ORDER BY c.name, COALESCE(i.item_code, sl.item_id::text);

-- View for stock movements with details (FIXED - removed is_active filter on items)
...
SELECT 
    ...
    c.name as company_name,
    sm.item_id,
    COALESCE(i.item_code, sm.item_id::text) as item_code,
    COALESCE(i.name, 'Unknown Item') as item_name,
    sm.movement_type,
    sm.movement_source,
    sm.reference_type,
