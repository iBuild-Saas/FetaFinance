...
BEGIN
    ...
    SELECT 
        si.item_id,
        COALESCE(i.item_code, i.name) as item_code,
        i.name as item_name,
        si.current_quantity,
...
COMMENT ON FUNCTION get_stock_levels IS 'Returns current stock levels for all items in a company';
COMMENT ON FUNCTION create_stock_movement IS 'Creates stock movement and updates inventory levels';

SELECT 'Quick Inventory System Setup Complete!' as status;
