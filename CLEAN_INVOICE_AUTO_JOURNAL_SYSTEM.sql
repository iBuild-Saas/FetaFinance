-- Clean Invoice Auto Journal Entry System with Customer/Supplier Tracking
-- Drops existing functions and creates clean versions
...
-- Drop all existing invoice journal functions to avoid conflicts
DROP FUNCTION IF EXISTS create_sales_invoice_journal_entry(UUID);
DROP FUNCTION IF EXISTS create_purchase_invoice_journal_entry(UUID);
DROP FUNCTION IF EXISTS trigger_sales_invoice_journal_entry();
DROP FUNCTION IF EXISTS trigger_purchase_invoice_journal_entry();
...
BEGIN
    ...
    INSERT INTO journal_entries (
        ...
        'Auto-generated from sales invoice: ' || v_invoice.invoice_number,
        'POSTED',
        'SALES_INVOICE',
        v_invoice.id,
        true
    ...
    -- Create AR debit line (total amount)
    ...
    -- Create sales revenue credit line (total amount)
    INSERT INTO journal_entry_lines (
        id,
        journal_entry_id,
        line_number,
        ...
        v_journal_entry_id,
        v_line_number,
        (SELECT id FROM chart_of_accounts WHERE company_id = v_invoice.company_id AND account_code = '4110' LIMIT 1),
...
BEGIN
    ...
    INSERT INTO journal_entries (
        ...
        'Auto-generated from purchase invoice: ' || v_invoice.invoice_number,
        'POSTED',
        'PURCHASE_INVOICE',
        v_invoice.id,
        true
    ...
    -- Create expense/inventory debit line (total amount)
    INSERT INTO journal_entry_lines (
        id,
        journal_entry_id,
        line_number,
        ...
        v_line_number,
        COALESCE(
            (SELECT id FROM chart_of_accounts WHERE company_id = v_invoice.company_id AND account_code = '1130' LIMIT 1), -- Inventory
            (SELECT id FROM chart_of_accounts WHERE company_id = v_invoice.company_id AND account_code = '5110' LIMIT 1)  -- Expense
        ),
        'Purchase: ' || v_invoice.invoice_number,
        v_invoice.total_amount,
        0,
        'PURCHASE_INVOICE',
        v_invoice.id
    ...
    -- Create AP credit line (total amount)
    INSERT INTO journal_entry_lines (
        id,
        journal_entry_id,
        line_number,
...
SELECT 'Clean invoice auto journal entry system created successfully' as status;
